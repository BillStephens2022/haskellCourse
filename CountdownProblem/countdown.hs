...
-- Rules
-- all the numbers, including intermediate results, must be positive naturals (1,2,3,...) (integers > 0)
...
-- an updated validation check to improve performance and filter out unneeded results
valid' :: Op -> Int -> Int -> Bool
valid' Add x y = x <= y  -- this ensures we don't get results for the same numbers but where order is different i.e. Add 1 2 and Add 2 1 are essentially same so don't need both in the results
valid' Sub x y = x > y
valid' Mul x y = x /= 1 && y /= 1 && x <= y  -- don't want results where expression multiplies a number by 1
valid' Div x y = y /= 1 && x `mod` y == 0  -- don't want results where expression divides numbers by 1
